...
"""Tools for the Google Drive reader & SQL data agent.

Strictly implements the `negotiate_creds` and `read_drive_file` pattern from:
`google/adk-samples/core/python/oauth-user-consent-flow/app/tools.py`
plus DuckDB SQL querying over CSV / Google Sheets files in Google Drive.
"""
...
def negotiate_creds(tool_context: ToolContext) -> Credentials | dict:
    ...
    if cached_token:
        ...
        if isinstance(cached_token, dict):
            logger.debug(
                "Cached token is a dict — loading as Credentials object"
            )
            try:
                ...
                logger.error(
                    f"Error loading/refreshing cached credentials: {error}"
                )
...
def read_drive_file(file_id: str, tool_context: ToolContext) -> dict:
    ...
    Google Sheets (exported as CSV), and plain text / CSV files.
...
def list_drive_files(tool_context: ToolContext) -> dict:
    """List accessible files in the signed-in user's Google Drive using `negotiate_creds`."""
    creds = negotiate_creds(tool_context)
    if isinstance(creds, dict):
        return creds
    ...
    try:
        ...
        return {"status": "error", "message": f"Failed to list files: {e!s}"}


def query_drive_csv(file_id: str, sql_query: str, tool_context: ToolContext) -> dict:
    """Execute a DuckDB SQL query over a CSV or Google Sheet file in Google Drive.

    Always query from the table name `drive_data` in your SQL statement!
    Example: SELECT Product_Type, SUM(Net_Revenue_SGD) FROM drive_data GROUP BY Product_Type

    Args:
        file_id: The Google Drive file ID to query.
        sql_query: SQL SELECT query referencing `drive_data`.
    """
    file_res = read_drive_file(file_id, tool_context)
    if file_res.get("pending") or file_res.get("status") == "error":
        return file_res

    try:
        df = pd.read_csv(io.StringIO(file_res["content"]))
        con = duckdb.connect()
        con.register("drive_data", df)
        ...
        return {
            "status": "sql_error",
            "error_message": str(e),
            "hint": "Always query from table `drive_data`. Check column names and retry.",
        }
